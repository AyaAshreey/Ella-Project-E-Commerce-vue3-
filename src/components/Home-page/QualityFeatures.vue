<template>
  <div
    class="q-fearures pb-16 pt-9"
    style="background-color: rgba(128, 128, 128, 0.13)"
  >
    <v-container fluid>
      <v-row>
        <v-col v-for="card in cards" :key="card.title" class="px-2">
          <v-hover v-slot="{ isHovering, props }">
            ...
            >
              <div class="parent">
                <div><span v-html="card.icon"></span></div>
                <v-card-tilte
                  style="font-weight: 700; text-transform: uppercase"
                  >{{ card.title }}</v-card-tilte
                >
